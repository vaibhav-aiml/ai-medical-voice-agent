...
describe('Prompts & Clinical Telemedicine Protocols', () => {
  it('should generate specific clinical guidelines in system prompts', () => {
    ...
    expect(generalPrompt).toContain('MEDICATION GUARDRAIL');
    expect(generalPrompt).toContain('contraindication check');
    ...
    expect(orthoPrompt).toContain('stomach ulcers, kidney impairment, or NSAID allergy');
  });

  it('should generate meaningful targeted fallback for lifting/workout injuries with contraindications', () => {
    ...
    expect(response).toContain('provided you have no history of stomach ulcers, kidney impairment, or NSAID allergy');
  ...
  it('should generate structured medication fallback for general pain and fever with contraindications', () => {
    ...
    expect(response).toContain('stomach ulcers, kidney impairment, or NSAID allergy');
  });
});
