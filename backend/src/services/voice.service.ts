...
function init() {
  ...
  if (!groq && process.env.OPENAI_API_KEY && process.env.OPENAI_API_KEY !== 'your-openai-api-key-here') {
    try {
      ...
      logger.warn('Failed to initialize OpenAI');
    }
  }

  if (!groq && !openai) {
    logger.warn('No AI provider available, using mock responses');
    useMockAI = true;
  }
}
...
function getSpecialistPrompt(specialistType: string): string {
...
function getFallbackReport(transcript: string): string {
...
export async function processVoiceStream(audioBuffer: Buffer, consultationId: string) {
  try {
...
export async function generateMedicalReport(consultationId: string, transcript: string, aiResponses: string) {
  if (!useMockAI && (groq || openai)) {
    try {
      ...
      return responseText || getFallbackReport(transcript);
    } catch (error) {
