...
class ReportGenerator {
  ...
  generateSOAPReport(data: ConsultationReport): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        // Add all content on the same page flow
        this.addHeader(doc, data);
        this.addPatientInfo(doc, data);
        this.addSubjective(doc, data.soapData);
        this.addObjective(doc, data.soapData);
        this.addAssessment(doc, data.soapData);
        this.addPlan(doc, data.soapData);
  ...
  private addHeader(doc: PDFKit.PDFDocument, data: ConsultationReport): void {
    doc.fontSize(20)
       .font('Helvetica-Bold')
       .fillColor('#1e3a8a')
  ...
  private addPatientInfo(doc: PDFKit.PDFDocument, data: ConsultationReport): void {
    doc.fontSize(12)
       .font('Helvetica-Bold')
       .fillColor('#1e293b')
  ...
  private addSubjective(doc: PDFKit.PDFDocument, soapData: SOAPData): void {
    this.addSectionHeader(doc, 'S', 'SUBJECTIVE', '#3b82f6');
  ...
  private addObjective(doc: PDFKit.PDFDocument, soapData: SOAPData): void {
    this.addSectionHeader(doc, 'O', 'OBJECTIVE', '#10b981');
  ...
  private addAssessment(doc: PDFKit.PDFDocument, soapData: SOAPData): void {
    this.addSectionHeader(doc, 'A', 'ASSESSMENT', '#f59e0b');
  ...
  private addPlan(doc: PDFKit.PDFDocument, soapData: SOAPData): void {
    this.addSectionHeader(doc, 'P', 'PLAN', '#8b5cf6');
  ...
  private addSectionHeader(doc: PDFKit.PDFDocument, letter: string, title: string, color: string): void {
    const y = doc.y;
