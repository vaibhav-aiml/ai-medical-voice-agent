...
function checkRedFlag(redFlag: string, symptom: string, severity: string, duration: string): boolean {
  if (severity === 'severe') return true;
  if (duration && parseDuration(duration) > 7) return true;
  return false;
}

function getChronicConditionRisks(patientProfile: PatientProfile): string[] {
  const risks: string[] = [];
...
function calculateUrgency(
  ...
  const chronicConditions = getChronicConditionRisks(patientProfile);
...
function getPatientFactorBonus(condition: string, patientProfile: PatientProfile): number {
  let bonus = 0;
...
function getRequiredTests(condition: string): string[] {
...
function generateReasoning(condition: string, symptoms: string[]): string {
...
function generateDifferentialDiagnoses(
  ...
  for (const [condition, score] of Array.from(matchedConditions.entries())) {
    const probability = Math.min(95, score * 20 + getPatientFactorBonus(condition, patientProfile));
    
    let confidence: 'high' | 'medium' | 'low' = 'medium';
    if (probability > 70) confidence = 'high';
    else if (probability < 40) confidence = 'low';
    
    const reasoning = generateReasoning(condition, supportingSymptoms[condition]);
    ...
    diagnoses.push({
      ...
      conflictingSymptoms: [],
      requiredTests: getRequiredTests(condition),
    });
  }
...
function generateSelfCare(diagnosis: string, symptoms: string[]): string[] {
...
function getWhenToSeeDoctor(urgency: string, redFlags: string[]): string {
  if (urgency === 'emergency') {
...
function identifyChronicCondition(symptoms: string[], patientProfile: PatientProfile): string {
...
function getChronicRecommendations(riskLevel: string, riskFactors: string[]): string[] {
  const recommendations: string[] = [
...
function getScreeningFrequency(riskLevel: string): string {
  switch (riskLevel) {
...
function getNextSteps(riskLevel: string): string[] {
  const steps: string[] = [];
...
function assessChronicRisk(symptoms: string[], patientProfile: PatientProfile): ChronicRiskAssessment | undefined {
  ...
  if (riskScore > 0) {
    const condition = identifyChronicCondition(symptoms, patientProfile);
    return {
      condition,
      riskScore,
      riskLevel,
      factors: riskFactors,
      recommendations: getChronicRecommendations(riskLevel, riskFactors),
      screeningFrequency: getScreeningFrequency(riskLevel),
...
function getSpecialistReferral(diagnosis: string): string | undefined {
...
export function analyzeSymptoms(
  ...
  for (const symptom of symptoms) {
    const symptomKey = normalizeSymptom(symptom);
    ...
    if (symptomData) {
      ...
      for (const redFlag of symptomData.redFlags) {
        if (checkRedFlag(redFlag, symptom, severities[symptom], durations[symptom])) {
  ...
  const urgencyLevel = calculateUrgency(redFlags, severities, patientProfile);
  ...
  const recommendations = generateRecommendations(primaryDiagnosis, urgencyLevel, patientProfile);
  const selfCare = generateSelfCare(primaryDiagnosis, symptoms);
  const riskAssessment = assessChronicRisk(symptoms, patientProfile);
  const specialistReferral = getSpecialistReferral(primaryDiagnosis);
  ...
  return {
    ...
    recommendations,
    selfCare,
    whenToSeeDoctor: getWhenToSeeDoctor(urgencyLevel, redFlags),
    riskAssessment,
